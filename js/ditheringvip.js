...
/**
 * Ostromoukhov 抖动 (简化版，使用固定内核)
 * 注意：完整的 Ostromoukhov 算法需要根据像素亮度动态调整系数，
 * 这里提供一个固定内核的简化版本作为高性能替代 
 */
...
/**
 * Linear Light Sierra Lite 抖动
 * 在线性光空间中进行误差扩散，处理前后进行 gamma 校正
 * 使误差分配更符合人眼感知，减少高光和阴影区域的噪点 
 */
function linearLightSierraDither(imageData, strength, colorMode) {
    ...
    // 1. 将 sRGB 转换到线性光空间 (使用预计算的查找表加速)
    for (let i = 0; i < pixelCount; i++) {
        const offset = i * 4;
        linearR[i] = _srgbToLinear[data[offset]];
        linearG[i] = _srgbToLinear[data[offset + 1]];
        linearB[i] = _srgbToLinear[data[offset + 2]];
    }

    // 将线性数据缩放到 0-255 范围并存入临时 ImageData
    const tempData = new Uint8ClampedArray(data.length);
    for (let i = 0; i < pixelCount; i++) {
        const offset = i * 4;
        tempData[offset] = Math.min(255, Math.max(0, Math.round(linearR[i] * 255)));
        tempData[offset + 1] = Math.min(255, Math.max(0, Math.round(linearG[i] * 255)));
        tempData[offset + 2] = Math.min(255, Math.max(0, Math.round(linearB[i] * 255)));
